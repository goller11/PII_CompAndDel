...
namespace CompAndDel.Filters {
    public class FilterConvolution : IFilter {
        ...
        public FilterConvolution (IConvolution matriz) 
        {
            this.matriz = matriz;
        }
        ...
        private Color ObtenerColorFiltrado (Color[, ] matrizVecinos) {
            ...
            for (int x = 0; x < matrizVecinos.GetLength (0); x++) {
                for (int y = 0; y < matrizVecinos.GetLength (1); y++) {
                    redFinal += matrizVecinos[x, y].R * this.matriz.MatrizConvolution[x, y];
                    greenFinal += matrizVecinos[x, y].G * this.matriz.MatrizConvolution[x, y];
                    blueFinal += matrizVecinos[x, y].B * this.matriz.MatrizConvolution[x, y];
                }
            }
            redFinal = Math.Abs ((redFinal / this.matriz.Divisor) + this.matriz.Complemento);
            redFinal = Math.Min (255, redFinal);

            greenFinal = Math.Abs ((greenFinal / this.matriz.Divisor) + this.matriz.Complemento);
            greenFinal = Math.Min (255, greenFinal);

            blueFinal = Math.Abs ((blueFinal / this.matriz.Divisor) + this.matriz.Complemento);
